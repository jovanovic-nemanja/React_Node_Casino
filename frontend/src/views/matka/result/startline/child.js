...
import {  getData, menusave, menuupdate, menudelete,allresult,todayresult ,revenuCalc} from "../../../../redux/actions/matka/result/startline"
...
import {Onlydate,get_options} from "../../../../redux/actions/auth"
...
class Child extends Component {
    ...
    rowEdit(row){
      ...
      this.setState({
        ...
        filters : filters,
        startLinetimer: row.startLinetimer
      });
      this.Bzarchange(row.bazaarid._id)
    }
    ...
    Change_result =  async (number) =>{
      ...
      this.setState({openresult : number,jodiresult : mod})
    ...
    dateChange =  async (date) => {
      ...
      if (this.state.openresult.length) {
    ...
    Bzarchange = async (bazarid) => { 
      const {bazaarlist} = this.props.dataList;
      let item = bazaarlist.find(obj=> obj.value === bazarid);
      console.log(this.props.dataList)
      if (item) {
        let timeroptions = [];
        let options = get_options(item.timer);
        for (let i in options) {
          timeroptions.push({label : options[i] , value : options[i]});
        }
        this.setState({timeroptions : timeroptions,  bazaarid : bazarid})
    ...
    timerChange = (timer) => {
      this.setState({ startLinetimer: timer })
    }
    ...
    render() {
      let { columns, data,totalPages, update,timeroptions ,numberoptions} = this.state;
              ...
              <ModalBody className="modal-dialog-centered mt-1 d-block">
                <Row>
...

                  {
                    ...
                    <Col md='12' className='mt-1'>
                      ...
                      <Select className="React" classNamePrefix="select"  name="select Bazaar" options={bazaarlist}
                        value={bazaarlist.find(obj => obj.value === this.state.bazaarid)} onChange={e => this.Bzarchange(e.value)} />
                    </Col>
                    : null
                  }
                  {
                  ...
                  }
                  {
                    !update ?
                    <Col md='12' className='mt-1'>
                      <Label for="startLinetimer">startLinetimer</Label>
                      <Select className="React" classNamePrefix="select"  name="startLinetimer" options={timeroptions}
                        value={timeroptions.find(obj => obj.value === this.state.startLinetimer)} onChange={e => this.timerChange(e.value)} />
                    </Col>
                    : null
                  }
                  {
                    update ? 
                    <Col  md="12">
                        <Label for="StartLine">StartLine Timer</Label>
                        <Input type="text" name="StartLine" id="StartLine" placeholder="StartLine"
                            disabled={true}
                            value = {timeroptions.find(obj => obj.value === this.state.startLinetimer) ? timeroptions.find(obj => obj.value === this.state.startLinetimer).label : ""}
                        />
                    </Col>
                    : null
                  }
...
const mapStateToProps = state => {
  return {
    dataList: state.matka.Result.startline,
  }
}
