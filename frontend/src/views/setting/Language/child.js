...
import {  UncontrolledDropdown, DropdownMenu, DropdownToggle, DropdownItem, Col, Row, Button, Modal, ModalHeader, ModalBody, ModalFooter, FormGroup, Label, Form } from "reactstrap"
import {  ChevronDown, ChevronLeft, ChevronRight, Trash, ArrowDown, ArrowUp, } from "react-feather"
import {  getData, menusave, menuupdate, menudelete } from "../../../redux/actions/settting/language"
...
const ActionsComponent = props => {
  return (
    <div className="data-list-action">
      ...
      {/* <Edit  classNam="cursor-pointer mr-1" size={20} onClick={()=>props.rowEdit(props.row)} /> */}
...
class Child extends Component {
    static getDerivedStateFromProps(props, state) {
        ...
        return {
            ...
            options: props.dataList.options,
        }
    }
    return null;
  }
    ...
    rowEdit = (row) => {
      this.setState({modal : true,code : row.code,rowid : row._id,update : true,ordernum : row.order,  text : row.text});
    }

  
    render() {

        let { columns, data,totalPages, rowsPerPage, totalRecords, sortIndex ,options} = this.state;
        return (
                            ...
                            <Col md="12">
                                ...
                                <FormGroup className="form-label-group">
                                    <Select
                                        ...
                                        name="code"
                                        options={options}
                                        value={options.find(obj => obj.value === this.state.code)}
                                        onChange={e => this.setState({ code: e.value, text : e.label })}
                                    />
                                </FormGroup>
                            </Col>
...
const mapStateToProps = state => {
    return {
        dataList: state.setting.language
    }
}
