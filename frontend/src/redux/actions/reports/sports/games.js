...
export const getTotal = (filters) => {
  return async dispatch =>{
    ...
    let rdata = await AXIOS_REQUEST("reports/sportsBygamesTotal",{ filters},dispatch,true);
    if(rdata.status){
      dispatch({
        type: ReposrtSportsGamesGetData,
        data: rdata.data
      });
    }else{
      alert("error","error")
...
export const getData = (params,filters) => {
  return  async(dispatch) => {
    ...
    filters['dates']["start"] = new Date(filters['dates']["start"]).toDateString();
    filters['dates']["end"] = new Date(filters['dates']["end"]).toDateString();
    let rdata = await AXIOS_REQUEST("reports/sportsBygameshistory",{filters,params},dispatch,true);
      if(rdata.status){
        var data = rdata.data;
