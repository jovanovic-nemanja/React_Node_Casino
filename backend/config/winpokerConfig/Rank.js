...
async function WinpokerTexasRank(boards , cards){
  
  var possible_boards = [
    [boards[0] , boards[1] , boards[2] , boards[3]],
    [boards[0] , boards[1] , boards[2] , boards[4]],
    [boards[0] , boards[1] , boards[3] , boards[4]],
    [boards[0] , boards[2] , boards[3] , boards[4]],
    [boards[1] , boards[2] , boards[3] , boards[4]],
  ];
  ...
  for (var i = 0; i < possible_hands.length; i++) {
    for (var j = 0; j < possible_boards.length; j++)
    {
      ...
      var tmp_points = await rankHandInt({cards : tmp_cards.toString()});

      if (tmp_points.rank > maxPoints.rank) {
        maxPoints = tmp_points;
        best_cards = tmp_cards;
      }
    }
  }
  return { rank : maxPoints.rank , rankname : maxPoints.message , best_cards };
}
