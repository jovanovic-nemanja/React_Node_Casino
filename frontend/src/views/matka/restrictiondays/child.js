...
import {  getData,save,update ,Delete } from "../../../redux/actions/matka/restrictiondays"
...
class ListViewConfig extends Component {
    ...
    let { columns,data,totalPages } = this.state;
      ...
      let {options} = this.props.dataList
    return (
      <React.Fragment>
        ...
        >
            <Form onSubmit={this.accontsave} action="#" >
                ...
                <Col md="12">
                  <FormGroup>
                    <Label for="gender">Bazars </Label>
                    <Select
                        className="React" classNamePrefix="select" id="ownerShip" name="ownerShip"
                        options={options} value={options.find(obj => obj.value === this.state.bazaarid)}
                        onChange={e => this.setState({ bazaarid: e.value })} />
                  </FormGroup>
                </Col>
...
const mapStateToProps = state => {
  return {  
    dataList: state.matka.restrictiondays
